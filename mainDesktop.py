...
class AudioStreamer:
    ...
    def _convert_audio(self, audio_data, current_sample_rate):
        """
        Convert audio data to 24kHz PCM 16-bit mono format
        """
        # Ensure mono channel
        ...
        if current_sample_rate != 24000:
            audio_data = resampy.resample(audio_data, current_sample_rate, 24000)
        
        # Normalize the audio data to [-1, 1] range
    ...
    async def send_audio(self, ws):
        ...
        # Record directly at 24kHz if possible
        sample_rate = 24000
        required_samples = int(sample_rate * self.chunk_duration)
        
        try:
            with self.microphone.recorder(samplerate=sample_rate, channels=1, blocksize=required_samples) as mic:
                while True:
                    try:
                        # Read audio chunk
                        ...
                        pcm_audio = self._convert_audio(audio_chunk, sample_rate)
            ...
            print(f"Recorder setup error: {e}")
